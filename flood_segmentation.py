...
if __name__ == '__main__':
    ...
    unet = UNet(n_channels=3, n_classes=2).to('cuda')
    ...
    pyplot.savefig('loss_unet.png')

    pyplot.clf()
    pyplot.imshow(pred_unflatten[2].to('cpu').detach().numpy())
    pyplot.savefig('pred_unet.png')
    pyplot.clf()
    pyplot.imshow(torch.permute(image[2].to('cpu'), (1, 2, 0)))
    pyplot.savefig('image_unet.png')
    pyplot.clf()
    pyplot.imshow(target[2][0].to('cpu').detach().numpy())
    pyplot.savefig('target_unet.png')

    pyplot.clf()
    pyplot.imshow(torch.permute(image[2].to('cpu'), (1, 2, 0)))
    pyplot.imshow(pred_unflatten[2].to('cpu').detach().numpy(), alpha=0.3)
    pyplot.savefig('overlay_unet.png')

    pyplot.clf()
    pyplot.plot(jaccard_list)
    pyplot.xlabel('Epoch')
    pyplot.ylabel('Jaccard Index')
    pyplot.title('Jaccard Index vs Epoch')
    pyplot.savefig('jaccard_unet.png')